...
object Database {
  ...
  hikariConfig.setJdbcUrl("jdbc:db2://54.237.14.75:50000/pebnch")
...
class OracleDMLSimulation extends Simulation {
  ...
  val insertOperation = trackQuery(
    "INSERT INTO pebnch.test_table (id, name, age) VALUES (?, ?, ?)",
    Map("1" -> "${id}", "2" -> "John Doe", "3" -> 30),
    "Insert"
  )

  val updateOperation = trackQuery(
    "UPDATE pebnch.test_table SET name = ? WHERE id = ?",
    Map("1" -> "Jane Doe", "2" -> "${id}"),
    "Update"
  )

  val deleteOperation = trackQuery(
    "DELETE FROM pebnch.test_table WHERE id = ?",
    Map("1" -> "${id}"),
    "Delete"
  )
